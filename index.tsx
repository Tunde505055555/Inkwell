...
const DEFAULT_CONTRACT = "0xdC6595506b8542d94f2d305464367bED7434dE9F";

type TxRecord = { hash: string; label: string; ts: number };
type StoryBattleState = { submissions?: unknown[]; is_judged?: boolean };
...
function Inkwell() {
  ...
  const onSubmitStory = () => {
    ...
    send("submit_story", [story.trim()], "Submit story");
  };
  ...
  const onJudge = async () => {
    ...
    try {
      ...
      setBusy(null);
      setJudging(true);
      const verdict = await pollJudgeResult(contractAddress, 180_000);
      if (verdict) {
        setJudgeResult(verdict);
        toast.success("AI verdict in", { description: shortAddr(verdict.winner) });
      } else {
        setJudgeError(
          "The judge transaction was submitted, but no parseable AI verdict is available yet. Check the transaction execution in the explorer, then try triggering the judge again if the round was not completed.",
        );
      }
    } catch (e: unknown) {
      toast.error(getErrorMessage(e, "Transaction failed"));
            ...
            <div className="rounded-3xl border border-accent/40 bg-gradient-to-br from-accent/10 via-card/70 to-card/40 p-6 backdrop-blur-sm shadow-[var(--shadow-card)] sm:p-8">
              ...
              {judgeResult && (
                <div className="mt-6 grid gap-5 lg:grid-cols-5">
                  <div className="lg:col-span-2 rounded-2xl border border-border bg-background/40 p-5">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <code
                        ...
                        title={judgeResult.winner}
                      >
                        {shortAddr(judgeResult.winner) || "—"}
                      </code>
                    </div>
                    {judgeResult.reason && (
                  ...
                  <div className="lg:col-span-3 rounded-2xl border border-border bg-background/40 p-5">
                    <div className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
                      Scorecard
                    </div>
